...
def train(train_loader, model, criterion, optimizer, epoch,is_fix):    
    ...
    for i, (input, target,_) in enumerate(train_loader):
        ...
        # compute output
        start = time.time()
        logits,feats = model(input,args.coarse_att,args.coarse_adj)
        inf_time = time.time()-start
        
        # compute gradient and do SGD step
        start = time.time()
        losses = criterion(target,logits,feats)
        optimizer.zero_grad()
        losses[0].backward()
        optimizer.step()
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for i, (input, target_c,target_f) in enumerate(val_loader):
            ...
            ''' sub '''
            # inference
            logits,feats = model(input,args.fine_att,args.fine_adj)
            ...
            sub_acc1.update(prec1[0], input.size(0))
            # opt
            ...
            sub_acc2.update(prec1[0], input.size(0))
            sub_acc3.update(prec5[0], input.size(0))
       
    print(' * Basic ACC {:.3f} Sub ACC1@ {:.3f} ACC2@ {:.3f} ACC3@ {:.3f}'.format(bas_acc.avg,sub_acc1.avg,sub_acc2.avg,sub_acc3.avg))

    return sub_acc2.avg.cpu().numpy()
