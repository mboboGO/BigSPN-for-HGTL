...
class Model(nn.Module):
    ...
    def forward(self, x, sf):
        ...
        sf_rec = self.rec(classifier)
        
        return (logit,logit_aux),(last_conv,sf,sf_rec)
...
class Loss(nn.Module):
    ...
    def forward(self, label, logits,feats):
        ...
        ''' Loss '''
        L_com = self.cls_loss(logit,label)
        
        L_cls = self.cls_loss(logit_aux,label)
        
        L_rec = 200*self.mse_loss(sf_rec,sf)
        
        total_loss = L_com + L_cls+L_rec
		
        return [total_loss,L_com,L_cls,L_rec]
		
def rth(pretrained=False, loss_params=None, args=None):
