...
class Model(nn.Module):
    def __init__(self, pretrained=True, args=None):
        ...
        self.ste_spa5 =  nn.Sequential(
            nn.Conv2d(cps_dim, cps_dim, kernel_size=3, stride=1, padding=1,bias=False),
            nn.ReLU(inplace=True),   
            nn.Conv2d(cps_dim, 1, kernel_size=1, stride=1, padding=0,bias=False),
            nn.Sigmoid(),        
        )
        self.ste_spa6 =  nn.Sequential(
            nn.Conv2d(cps_dim, cps_dim, kernel_size=3, stride=1, padding=1,bias=False),
            nn.ReLU(inplace=True),   
            nn.Conv2d(cps_dim, 1, kernel_size=1, stride=1, padding=0,bias=False),
            nn.Sigmoid(),        
        )
        self.ste_spa7 =  nn.Sequential(
            nn.Conv2d(cps_dim, cps_dim, kernel_size=3, stride=1, padding=1,bias=False),
            nn.ReLU(inplace=True),   
            nn.Conv2d(cps_dim, 1, kernel_size=1, stride=1, padding=0,bias=False),
            nn.Sigmoid(),        
        )
        self.ste_spa8 =  nn.Sequential(
            nn.Conv2d(cps_dim, cps_dim, kernel_size=3, stride=1, padding=1,bias=False),
            nn.ReLU(inplace=True),   
            nn.Conv2d(cps_dim, 1, kernel_size=1, stride=1, padding=0,bias=False),
            nn.Sigmoid(),        
    ...
    def forward(self, x, p_atr,s_atr):
        ...
        att5 = self.ste_spa5(x)
        att6 = self.ste_spa6(x)
        att7 = self.ste_spa7(x)
        att8 = self.ste_spa8(x)
        # att
        x_att1 = x*att1
        x_att2 = x*att2
        x_att3 = x*att3
        ...
        x_att7 = x*att7
        x_att8 = x*att8
        x = torch.cat([x_att1,x_att2,x_att3,x_att4,x_att5,x_att6,x_att7,x_att8],dim=1)
...
def as5(pretrained=False, loss_params=None, args=None):
