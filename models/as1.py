...
class Model(nn.Module):
    ...
    def forward(self, x, p_atr,s_atr):
        ...
        ''' Sub-grained Transfer Embedding '''
        x = self.pse_proj(last_conv).view(x.size(0),-1)
        # trans
...
class Loss(nn.Module):
    ...
    def forward(self, label, logits):
        ...
        logit_ste = (logit_ste*idx).sum(dim=1)/idx.sum(dim=1)
        L_trans = (1-logit_ste).mean()
        
        return L_cls,L_sem,L_trans
		
def as1(pretrained=False, loss_params=None, args=None):
